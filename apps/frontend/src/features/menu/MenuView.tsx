...
export function MenuView() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-40">
      ...
      {/* Search */}
      <div className="px-4 py-3 sticky top-[52px] sm:top-[60px] z-30 bg-slate-950/95 backdrop-blur-md border-b border-slate-800/50">
        <div className="relative">
      ...
      {/* Menu Grid */}
      <div className="px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
        {filteredItems.map((dish) => {
          ...
          return (
            <div
              key={dish.ID_Plato}
              className={`bg-slate-800/50 backdrop-blur rounded-2xl overflow-hidden border transition-all duration-300 ${
      ...
      {/* Fixed Bottom Container for Banner and Cart */}
      <div className="fixed bottom-4 left-4 right-4 z-30 flex flex-col items-center gap-3 pointer-events-none">
        {/* Suggestion banner */}
        {cartCount > 0 && cartCount < 3 && (
          <div className="pointer-events-auto bg-slate-800/95 backdrop-blur rounded-xl border border-primary/20 p-3 flex items-center gap-3 shadow-xl max-w-lg mx-auto w-full">
            <Star className="w-5 h-5 text-primary shrink-0" />
            <p className="text-xs text-slate-300">
              <span className="text-primary font-semibold">¿Quieres completar tu experiencia?</span> Agrega una bebida o postre y obtén una experiencia completa.
            </p>
          </div>
        )}
        ...
        {cartCount > 0 && (
          <button
            onClick={toggleCart}
            className="pointer-events-auto w-full max-w-lg mx-auto bg-primary hover:bg-primary/90 text-on-primary rounded-2xl py-4 px-6 flex items-center justify-between font-bold shadow-2xl shadow-primary/20 active:scale-[0.98] transition-all"
          >
