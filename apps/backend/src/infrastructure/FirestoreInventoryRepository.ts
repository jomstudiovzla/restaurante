...
import { IInventoryRepository } from '../domain/entities';
...
export class FirestoreInventoryRepository implements IInventoryRepository {
  ...
  async deductIngredients(ingredientsToDeduct: { ingredientId: string; amount: number; }[]): Promise<void> {
    ...
    await this.db.runTransaction(async (transaction) => {
      ...
      const inventoryRefs = ingredientsToDeduct.map(req => 
        this.db.collection('inventory').doc(req.ingredientId)
      );

      const inventoryDocs = await transaction.getAll(...inventoryRefs);

      // Verify and calculate new stock
      const updates: { ref: admin.firestore.DocumentReference, newStock: number }[] = [];
